...
class DistillationIQASolver(object):
    def __init__(self, config):
        ...
        self.studentNet = DistillationIQANet_org_or_stackingV2(self_patch_num=config.self_patch_num, distillation_layer=config.distillation_layer , stacking_mode=config.feature_stacking, repeatable_loss = config.repeatable_loss)
        ...
        train_loader = DataLoader(config.train_dataset, folder_path[config.train_dataset], config.ref_train_dataset_path, config.train_index, config.patch_size, config.train_patch_num, batch_size=config.batch_size, istrain=True, self_patch_num=config.self_patch_num, repeatable_loss = config.repeatable_loss)
    ...
    def train(self):
        best_srcc_LIVE, best_srcc_CSIQ, best_srcc_TID, best_srcc_livec = 0.0, 0.0, 0.0, 0.0
        best_plcc_LIVE, best_plcc_CSIQ, best_plcc_TID, best_plcc_livec = 0.0, 0.0, 0.0, 0.0
        best_krcc_LIVE, best_krcc_CSIQ, best_krcc_TID, best_krcc_livec = 0.0, 0.0, 0.0, 0.0
        ...
        for t in range(self.epochs):
            ...
            for LQ_patches, refHQ_patches, LQ_patches2,  ref_patches, ref_patches2,  label in tqdm(self.train_data):
                LQ_patches, refHQ_patches, LQ_patches2,  ref_patches, ref_patches2,  label = LQ_patches.to(self.device), refHQ_patches.to(self.device), LQ_patches2.to(self.device), ref_patches.to(self.device), ref_patches2.to(self.device), label.to(self.device)
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # torchvision.utils.save_image(LQ_patches[0, 0], f"LQ_patches.png")
                    # torchvision.utils.save_image(refHQ_patches[0, 0], f"refHQ_patches.png")
                    # torchvision.utils.save_image(LQ_patches2[0, 0], f"LQ_patches2.png")
                    # torchvision.utils.save_image(ref_patches[0, 0], f"ref_patches.png")
                    # torchvision.utils.save_image(ref_patches2[0, 0], f"ref_patches2.png")

                    s_encode_diff_inner_feature, s_decode_inner_feature, final_lq_features1, final_diff_features1,  pred = self.studentNet(LQ_patches, ref_patches)
                    s_encode_diff_inner_feature, s_decode_inner_feature, final_lq_features2, final_diff_features2, pred = self.studentNet(LQ_patches2, ref_patches2)
                
                    pred_scores = pred_scores + pred.cpu().tolist()
                    gt_scores = gt_scores + label.cpu().tolist()
                    pred_loss = self.l1_loss(pred.squeeze(), label.float().detach())

                    encode_diff_loss, decode_loss , feature_loss = 0.0, 0.0, 0.0
                    ...
                    scaled_repeatable_loss = (self.mse_loss(final_diff_features1, final_diff_features2) + self.mse_loss(final_lq_features1, final_lq_features2)) * 100
                   

                    loss = pred_loss + feature_loss*self.feature_loss_ratio  + scaled_repeatable_loss


                    featureLosses.append(feature_loss)
                    predLosses.append(pred_loss)
                    sumLosses.append(loss)
            ...
            write_to_file(predLosses , "checkpoint_DistillationIQA/log/pred_loss.txt")
            write_to_file(featureLosses , "checkpoint_DistillationIQA/log/feature_loss.txt")
            write_to_file(sumLosses , "checkpoint_DistillationIQA/log/loss.txt")
