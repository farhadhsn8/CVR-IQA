...
class DistillationIQANet_makeRef(nn.Module):
    def __init__(self, self_patch_num=10, lda_channel=64, encode_decode_channel=64, MLP_depth=9, distillation_layer=9, stacking_mode=False):
        super(DistillationIQANet_makeRef, self).__init__()
        ...
        print("DistillationIQANet_makeRef...")
        self.lda1_process_lq = nn.Sequential(nn.Conv2d(256, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
        self.lda2_process_lq = nn.Sequential(nn.Conv2d(512, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
        self.lda3_process_lq = nn.Sequential(nn.Conv2d(1024, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
        self.lda4_process_lq = nn.Sequential(nn.Conv2d(2048, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
        
        self.lda_process_lq = [self.lda1_process_lq, self.lda2_process_lq, self.lda3_process_lq, self.lda4_process_lq]

        self.lda1_process_hq = nn.Sequential(nn.Conv2d(256, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
        self.lda2_process_hq = nn.Sequential(nn.Conv2d(512, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
        self.lda3_process_hq = nn.Sequential(nn.Conv2d(1024, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
        self.lda4_process_hq = nn.Sequential(nn.Conv2d(2048, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
        
        self.lda_process_hq = [self.lda1_process_hq, self.lda2_process_hq, self.lda3_process_hq, self.lda4_process_hq]


        self.shared_net = nn.Sequential(nn.Conv2d(2560, 1024, kernel_size=1, stride=1, padding=0))



        self.MLP_encoder_diff = MLPMixer(image_size = 7, channels = self.lda_channel*16, patch_size = 1, dim = self.encode_decode_channel*4, depth = self.MLP_depth*2)
        ...
        initialize_weights(self.lda1_process_lq,0.1)
        initialize_weights(self.lda2_process_lq,0.1)
        initialize_weights(self.lda3_process_lq,0.1)
        initialize_weights(self.lda4_process_lq,0.1)

        initialize_weights(self.lda1_process_hq,0.1)
        initialize_weights(self.lda2_process_hq,0.1)
        initialize_weights(self.lda3_process_hq,0.1)
        initialize_weights(self.lda4_process_hq,0.1)


        initialize_weights(self.shared_net,0.1)
    ...
    def forward(self, LQ_patches, refHQ_patches):
        ...
        c = 0
        multi_scale_make_ref_lda_feature, multi_scale_lq_feature, feature = [], [], []
        for lq_lda_feature, refHQ_lda_feature, lda_process_lq, lda_process_hq in zip(lq_lda_features, refHQ_lda_features, self.lda_process_lq , self.lda_process_hq ):
            # [b*p, 256, 56, 56] --> [b, 2560, 56, 56]
         

            lq_lda_feature = lda_process_lq(lq_lda_feature).view(b, -1, 7, 7)  
            refHQ_lda_feature = lda_process_hq(refHQ_lda_feature).view(b, -1, 7, 7)
         
            make_ref_lda_feature = refHQ_lda_feature + lq_lda_feature
            
            
            multi_scale_make_ref_lda_feature.append(make_ref_lda_feature)
        ...
        multi_scale_make_ref_lda_feature = torch.cat(multi_scale_make_ref_lda_feature, 1).to(device)

        multi_scale_make_ref_lda_feature = self.shared_net(multi_scale_make_ref_lda_feature)
        shared_lq = self.shared_net(multi_scale_lq_feature)

        subtracted_ref_lq = shared_lq - multi_scale_make_ref_lda_feature


        encode_lq_feature, encode_lq_inner_feature = self.MLP_encoder_lq(multi_scale_lq_feature, self.distillation_layer_num)
        encode_diff_feature, encode_diff_inner_feature = self.MLP_encoder_diff(subtracted_ref_lq, self.distillation_layer_num)
...
class DistillationIQANet_org_or_stackingV2(nn.Module):
    def __init__(self, self_patch_num=10, lda_channel=64, encode_decode_channel=64, MLP_depth=9, distillation_layer=9, stacking_mode=False , repeatable_loss = False):
        ...
        if self.stacking_mode == True:
            print("stacking modeV2(combined)...")
            # self.lda1_process = nn.Sequential(nn.Conv2d(2560, 2 * self.lda_channel, kernel_size=1, stride=1, padding=0),
            #                                   nn.Conv2d(2 * self.lda_channel, 10 * self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
            self.lda1_process = nn.Sequential(nn.Conv2d(256, self.lda_channel, kernel_size=1, stride=1, padding=0), nn.AdaptiveAvgPool2d((7, 7)))
            self.lda2_process = nn.Sequential(nn.Conv2d(5120, 2 * self.lda_channel, kernel_size=1, stride=1, padding=0),
